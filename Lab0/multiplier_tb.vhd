...
begin
  ...
  begin
    ...
    -- Complete coverage of all 8-bit inputs would require 65536 tests, which is more computationally intensive
    -- lo sto facendo solo per i primi 16
      ...
      for j in 0 to 15 loop
        ...
        if p /= std_logic_vector(to_unsigned(ref, 16)) then
          report "FAILED: " & integer'image(i) & " x " &
            ...
            -- abbiamo detto error, quindi da errore ma continua
            severity error;
          errors := errors + 1;
        end if;
      end loop;
